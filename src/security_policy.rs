...
pub fn sanitize_download_path(req: &str, base: &Path) -> Result<PathBuf, String> {
    ...
    // Reject if basename itself contains path separators or NUL
    if fname_str.contains('/') || fname_str.contains('\\') || fname_str.contains('\0') {
        return Err(format!("rejected download name (separator/NUL): {:?}", fname_str));
    }
    // Extra safety: after file_name(), still ensure original didn't trick us with empty
    // The file_name check already strips dirs, but we keep the explicit contains check per spec.
