...
::-webkit-scrollbar-corner    { background: #0d1117 !important; }`;
    ...
    function buildBar() {
        ...
        flyout.innerHTML = `
            ...
            <span class="fly-feat">Tab isolation (multi-account)</span>
            <span class="fly-feat">Tor routing (Linux WebView / all downloads)</span>
            <span class="fly-feat">Reader mode</span>
            <span class="fly-feat">Device sync chain</span>
...
(function () {{
    ...
    try {{
        ...
        XMLHttpRequest.prototype.open = function (method, url, ...rest) {{
            if (_isAd(String(url))) {{
                console.debug('[Catisen] XHR blocked:', url);
