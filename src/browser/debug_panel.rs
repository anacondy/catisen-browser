...
#__cat_settings_panel .close-btn:hover { color:#e6edf3!important; background:#21262d!important; }`;
    ...
    panel.appendChild(row('Tab Isolation (experimental)',
        'Profile cleanup is tracked; separate WebView cookie contexts are not active yet',
        tog('cs_isolation', false, on => ipc({ t:'set_isolation', enabled:on }))));
    panel.appendChild(row('Persistent Visit History',
        'Onion visits are never persisted; disabling clears visits from disk',
        tog('cs_history', true, on => ipc({ t:'set_save_history', enabled:on }))));

    // ── Tor section ───────────────────────────────────────────────────────────
    panel.appendChild(sec('Tor Routing'));
    panel.appendChild(row('Tor routing',
        'WebView and downloads use the configured SOCKS route when available. ' +
        'Requires a reachable Tor daemon and browser restart for WebView changes.',
    ...
    // ── Permissions section ───────────────────────────────────────────────────
    // Use the exact origin so HTTP/HTTPS and non-default ports do not share a grant.
    ...
    panel.appendChild(row('Geolocation (this site)',
        ...
        tog('cs_geo', false, on => ipc({ t:'set_permission', domain: location.origin || '*', permission:'geolocation', state: on?'granted':'denied' }))));
